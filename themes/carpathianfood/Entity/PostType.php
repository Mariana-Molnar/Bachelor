...
class PostType {
	...
	public function loadField($field)
	{
		...
		if (method_exists($this,'load' . Helpers::camelcase($field))) {
			$this->{'load' . Helpers::camelcase($field)}();
		} else {
	...
	{
		if (!$force && $post->post_type != $this::$MACHINE_NAME) return $this;
	...
	{
		$method = 'load' . Helpers::camelcase($taxonomy_field);
		if (method_exists($this,$method)) return $this->{$method}();

		if (!property_exists($this,$taxonomy_field)) return $this;
		if (!in_array($taxonomy_name, $this->getTaxonomies())) return $this;
	...
	{
		$method = 'save' . Helpers::camelcase($taxonomy_field);
		if (method_exists($this,$method)) return $this->{$method}();
	...
	{
		$method = 'addTo' . Helpers::camelcase($taxonomy_field);
		if (method_exists($this,$method)) return $this->{$method}($termitem);
		...
		if (empty($term) || is_wp_error($term)) return $this;
