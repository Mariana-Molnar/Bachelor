...
class PostTypeCreator
{
	...
	{
		if (empty($file_name)) $file_name = $machine_name;

		$name = implode(DIRECTORY_SEPARATOR, [self::$entityDirName, ucfirst(strtolower($machine_name)), $file_name.'.json']);

		$file = locate_template([$name]);

//		$directory = implode(DIRECTORY_SEPARATOR, [get_stylesheet_directory(), self::$entityDirName, ucfirst(strtolower($machine_name))]);
//
